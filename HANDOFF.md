...
### 补记 12（同日）：mini 首发包定稿——《兰亭序》神龙本
- 用户拍板：弃九成宫/圣教序（上图 CC BY-NC-ND 审核顾虑 + 圣教序佛经内容敏感），改用王羲之《兰亭序》。YGSF 源检索（server/scripts/tmp-ygsf-search.ts 模式）命中 zitie c4fe6f21：506 卡/33 页，**正文起首即「蘭亭八柱苐三」题签——正是冯承素摹神龙本系统**。核验 464/506、0 错字，已放行并上架（市场名「王羲之兰亭序」）。
- mini 包定稿：兰亭序 282 独字（去重后），384px q85，18 个图集，全包 7.25MiB / 22 文件。包内已无上图内容 → 关于页改「原作已属公有领域」表述，CC 许可 json 从包内移除；集字模板换兰亭名句（永和九年/天朗气清/惠风和畅/茂林修竹，覆盖已验）。
- 核验工具两处通用 bug 修复（影响所有未来核验）：①翻页 `<100 即断` 截断稀疏页帖 ②idRe 指纹正则不认无下划线文件名（部分帖 `<hash>.png` 无 `_`，此前覆盖恒 0）。
...
### 补记 14（同日）：换包后"字仍被裁"——缓存同路径撞车，图集文件名加内容哈希
- contain 修复后用户预览仍见裁字。排查：新图集字节是完整的（提取 tile 与原图逐像素比对一致），用户浏览器渲染的是**旧 cover 图集**——重打包前后图集路径相同（`<z>-00.webp`），python http.server 无缓存头，Chrome 按 Last-Modified 启发式缓存直接命中旧字节，刷新都不回源。真机同理是隐患：容器换包若按路径缓存文件，用户永远看到旧图。
- 修复：图集文件名带内容 sha1 前 8 位（`<z>-00.a93bd6e8.webp`），内容一变 URL 就变，任何缓存层都不可能再给旧字节；manifest rel 随之生成，resyncBundledImages 按汉字刷旧地址（实测不清 IDB 直接刷新 → 卡片自动改用哈希地址 → 渲染完整）。封面内容跨包不变，保持原名。
- 用户侧一次性动作：普通刷新即可（index.html 会回源）；顽固时清一次站点数据，之后哈希文件名根治。
...
### 补记 21（同日）：删除版本暗示文案，主打「像背单词一样背字帖」
- 用户指出「离线版」「进度保存在本机」会让审核觉得存在其他版本/站外导流。清除四处：mini.html 页面标题「背字帖 · 离线版」→「背字帖」；主屏副标题 →「像背单词一样背字帖」（主打语）；清单 desc 去「· 离线版 · 纯文字」后缀；错误文案「该帖不在离线包内」→「该帖不在包内」。产物扫描：离线/本机/单文件/在线版/网页版 0 残留。

### 补记 22（2026-09-08）：放弃小红书小工具，下线 mini 全套
- 六连拒且零理由。包内全变量实验已穷尽：有图/无图、带字体/零字体文件、黑体宋体收敛、版本暗示词清除、Chrome 61 运行时白屏修复——全部被拒。结论：卡点在包外（名称占用/表单文案/账号主体/平台对第三方内容工具的开放度），继续改包无信息量，用户决定终止。
- 清理：README 摘除快速开始行、小工具章节、项目结构 mini/build-mini 行、霞鹜文楷致谢；CHANGELOG v1.2.0（未发布）摘除小红书节，补记两条单文件版真实修复（闪卡换卡泄题、缓存图串帧）；删除 mini/ 全套、build-mini.sh、vite.config.mini.ts、mini.html、App.mini.tsx、*.mini.tsx 空实现、package.json build:mini、main.tsx 的 mini-compat 调用；本地产物 dist-mini/beizitie-mini.zip/.cache 已清。
- 保留：git 历史全量（图集管线、字体收敛、审核排查方法论在补记 9-21）；共享代码中的 __MINI__ 守卫为惰性代码（single 构建期折叠为 false），保留以减小 diff。审核契约沉淀：≤10MiB/≤200 文件/单入口 html/Chrome 61 基线/JSBridge 白名单，若重启移动分发直接翻补记。
- 若重启移动分发的出口：微信小程序个人主体（工具类目可发，纯离线包几乎零适配）或 PWA。
