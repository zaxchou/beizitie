...
**背字帖**把历代碑帖拆成一张张单字高清卡，像背单词一样帮你背字：按艾宾浩斯遗忘曲线安排复习（Anki 同款 SM-2 算法），快忘记时刚好提醒你，直到烂熟于心。市场共上架 **1431 部经核验碑帖、38 万余张单字**（行 803 / 楷 265 / 草 270 / 隶 64 / 篆 31），覆盖楷行草隶篆、184 位书家，其中含上海图书馆藏本 45 部（带整卷原拓坐标）。
...
| 🔁 **科学记忆（间隔重复）** | 像背单词一样背字：快忘记时刚好提醒你，复习间隔 10 分钟 → 明天 → 4 天后越拉越长，直到烂熟于心；出卡顺序支持 到期优先 / 按帖序 / 随机 |
...
| 按书体 / 朝代筛选，编辑精选 | 真拓单字卡 + 复习评分 | 单字回看整卷位置与行气 |
...
| 📱 **小红书小工具** | 在小红书 App 内离线背帖，无需安装 | 已提交审核，见下文 |
...
单文件版的"塞进口袋"形态：把字帖数据压缩打包进小红书小工具，**纯离线运行、无需安装**。

- 首发：《兰亭序》282 独字（王羲之摹本，「兰亭八柱第三」，384px 图集）
- 审核极简版：整个工具只做背字一件事——主屏今日概览（新学 / 复习 / 已学 / 全帖）+ 完整 SM-2 学习，进度存本机
- 诊断版：字卡以系统楷体渲染，包内**零图片、零字体文件**，仅 1 个文件约 0.1MiB，用于定位平台拒审原因；字图图集管线（contain 完整放入、纸色补边、内容哈希文件名）以 config 开关保留，随时可恢复
- 里程碑字帖将做成系列包（隶书、小楷、行书手札…），换帖出包一条命令
...
├── mini/           # 小红书离线包：选帖配置、数据管线（图集打包）、内联数据与字图
├── docs/           # 封面设计稿、界面截图、维护 SOP、计划文档
├── beizitie.html   # 单文件版成品（构建产物，随 main 分发）
└── build-mini.sh   # 离线包一键构建（数据管线 → 构建 → 打包 → 门禁）
```
...
- **[霞鹜文楷 LXGW WenKai](https://github.com/lxgw/LxgwWenKai)** — 离线包界面字体（SIL OFL 1.1）
